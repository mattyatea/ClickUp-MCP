...
export class ClickUpFormatters {
   ...
   */
  async fetchTasksByIds(accessToken: string, ids: string[], teamId: string, page: number = 0, limit: number = 15) {
    const paginatedIds = ids.slice(page * limit, (page + 1) * limit);

    const taskPromises = paginatedIds.map(async (taskId: string) => {
      try {
        const taskResponse = await fetch(`${this.deps.config.clickupApiBaseUrl}/task/${taskId}`, {
          headers: {
