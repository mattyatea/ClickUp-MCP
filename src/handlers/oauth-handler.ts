 ...
 */
function createAdaptiveErrorResponse(request: Request, error: string, statusCode: number = 500, code?: string): Response {
 ...
 * ClickUpへのリダイレクト処理
 */
async function redirectToClickUp(request: Request, oauthReqInfo: AuthRequest, env: Env, headers: Record<string, string> = {}) {
 ...
 */
app.notFound((c) => {
  return createAdaptiveErrorResponse(c.req.raw, "お探しのページは見つかりませんでした。URLを確認してください。", 404, "not_found");
});
