            ...
            try {
                ...
                if (data.tasks.length === 0) {
                    ...
                    data.tasks.forEach((task: any, index: number) => {
                        const urgencyEmoji = task.priority === "urgent" ? "🚨" :
                            task.priority === "high" ? "🔴" :
...
- 統計情報・傾向分析`,
        {
            ...
            priorities: z.array(z.enum(['urgent', 'high', 'normal', 'low'])).optional().describe("優先度の配列（例: ['urgent', 'high']）"),
        ...
        async (args: {
            ...
            try {
                ...
                if (filters.priorities?.length) {
                    const priorityNames = filters.priorities.map(p => Object.keys(priorityMap).find(k => priorityMap[k as keyof typeof priorityMap] === p));
                ...
                if (data.tasks.length === 0) {
                    ...
                    data.tasks.forEach((task: any, index: number) => {
                        ...
                        // 担当者と作成者
                        const assigneesDisplay = task.assignees?.length > 0
