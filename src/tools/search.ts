...
- 期限切れ警告`,
		{
			...
			limit: z
				...
				.optional()
				.default(15)
				.describe("取得件数（デフォルト: 15、最大: 100）"),
			page: z
				.number()
				.optional()
				.default(0)
				.describe("ページ番号（0から開始）"),
		},
		{
			type: "object",
			properties: {
		...
		async ({
			...
			try {
				const data = await clickupClient.getMyTasks(
					getAccessToken(),
					teamId,
					limit,
					page,
...
- ページネーション対応`,
		...
		async ({
			...
			try {
				const data = await clickupClient.searchTasks(
					getAccessToken(),
					searchTerm,
					teamId,
					limit,
...
- 統計情報・傾向分析`,
		{
			...
			statuses: z
				.array(z.string())
				.optional()
				.describe("ステータス名の配列（例: ['open', 'in progress']）"),
			priorities: z
			...
			dueDateFrom: z
				.string()
				.optional()
				.describe("期限開始日（ISO8601形式: 2024-01-01T00:00:00Z）"),
			dueDateTo: z
				.string()
				.optional()
				.describe("期限終了日（ISO8601形式: 2024-01-31T23:59:59Z）"),
			startDateFrom: z
				.string()
				.optional()
				.describe("開始日の開始（ISO8601形式）"),
			startDateTo: z
				.string()
				.optional()
				.describe("開始日の終了（ISO8601形式）"),
			createdDateFrom: z
				.string()
				.optional()
				.describe("作成日の開始（ISO8601形式）"),
			createdDateTo: z
				.string()
				.optional()
				.describe("作成日の終了（ISO8601形式）"),
			updatedDateFrom: z
				.string()
				.optional()
				.describe("更新日の開始（ISO8601形式）"),
			updatedDateTo: z
				.string()
				.optional()
				.describe("更新日の終了（ISO8601形式）"),

			// タグフィルター
			tags: z
				.array(z.string())
				.optional()
				.describe("タグ名の配列（例: ['frontend', 'urgent']）"),

			// その他のオプション
			parentTaskId: z
				.string()
				.optional()
				.describe("親タスクID（サブタスクを検索）"),
			includeSubtasks: z
				.boolean()
				.optional()
				.default(true)
				.describe("サブタスクを含める（デフォルト: true）"),
			includeArchived: z
				.boolean()
				.optional()
				.default(false)
				.describe("アーカイブ済みタスクを含める（デフォルト: false）"),
			includeClosed: z
				.boolean()
				.optional()
				.default(false)
				.describe("完了済みタスクを含める（デフォルト: false）"),
				...
				tasks: {
					...
					items: {
						...
						properties: {
							...
							start_date_readable: {
								type: "string",
								description: "開始日（読みやすい形式）",
							},
							created_date_readable: {
								type: "string",
								description: "作成日（読みやすい形式）",
							},
							updated_date_readable: {
								type: "string",
								description: "更新日（読みやすい形式）",
							},
							list: {
								type: "object",
								properties: {
		...
		async (args: {
			...
			try {
				...
				const data = await clickupClient.searchTasksAdvanced(
					getAccessToken(),
					filters,
				);
				...
				if (filters.searchTerm)
					filterDescriptions.push(`🔍 キーワード: "${filters.searchTerm}"`);
				...
				if (filters.priorities?.length) {
					const priorityNames = filters.priorities.map((p) =>
						Object.keys(priorityMap).find(
							(k) => priorityMap[k as keyof typeof priorityMap] === p,
						),
					);
				...
				if (filters.assigneeIds?.length)
					filterDescriptions.push(
						`👤 担当者ID: ${filters.assigneeIds.join(", ")}`,
					);
				if (filters.tags?.length)
					filterDescriptions.push(`🏷️ タグ: ${filters.tags.join(", ")}`);
				if (filters.dueDateFrom || filters.dueDateTo) {
					const fromStr = filters.dueDateFrom
						? new Date(filters.dueDateFrom).toLocaleDateString()
						: "開始";
					const toStr = filters.dueDateTo
						? new Date(filters.dueDateTo).toLocaleDateString()
						: "終了";
					filterDescriptions.push(`📅 期限: ${fromStr} ～ ${toStr}`);
				}
				if (filters.parentTaskId)
					filterDescriptions.push(`🔗 親タスク: ${filters.parentTaskId}`);
				if (!filters.includeClosed) filterDescriptions.push(`❌ 完了済み除外`);
				if (filters.includeArchived)
					filterDescriptions.push(`📁 アーカイブ済み含む`);
				...
				if (data.tasks.length === 0) {
					...
					data.tasks.forEach((task: any, index: number) => {
						...
						// 日付情報
						const dateInfo = [];
						if (task.due_date_readable)
							dateInfo.push(`⏰ 期限: ${task.due_date_readable}`);
						if (task.start_date_readable)
							dateInfo.push(`🚀 開始: ${task.start_date_readable}`);
						if (task.created_date_readable)
							dateInfo.push(`📅 作成: ${task.created_date_readable}`);
						if (task.updated_date_readable)
							dateInfo.push(`🔄 更新: ${task.updated_date_readable}`);
						const dateDisplay =
							dateInfo.length > 0 ? `\n${dateInfo.join(" | ")}` : "";
						...
						const assigneesDisplay =
							task.assignees?.length > 0
								? `👥 担当者: ${task.assignees.map((a: any) => a.username).join(", ")}`
								: "👥 担当者: 未割り当て";

						const creatorDisplay = task.creator?.username
							? ` | 👤 作成者: ${task.creator.username}`
							: "";
				...
				throw new Error(
					`詳細タスク検索に失敗しました: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		},
	);
