...
export class ClickUpAdvancedSearch {
	...
	async searchTasksAdvanced(
		...
		try {
			...
			if (filters.teamId) {
				...
				teams = team
					? [team]
					: [{ id: filters.teamId, name: `Team ${filters.teamId}` }];
			} else {
				const workspacesData = await this.auth.getWorkspaces(accessToken);
			...
			for (const team of teams) {
				try {
					...
					if (!response.ok) {
						console.warn(
							`チーム ${team.id} の詳細検索でエラー: ${response.status}`,
						);
						continue;
					}
			...
			if (filters.customFields) {
				filteredResults = this.filterByCustomFields(
					searchResults,
	...
	private setSearchParameters(url: URL, filters: AdvancedSearchFilters) {
		...
		if (filters.createdDateFrom) {
			url.searchParams.set(
				"date_created_gt",
				filters.createdDateFrom.toString(),
			);
		}
		...
		if (filters.updatedDateFrom) {
			url.searchParams.set(
				"date_updated_gt",
				filters.updatedDateFrom.toString(),
			);
		}
	...
	async getAvailableStatuses(accessToken: string, spaceId: string) {
		try {
			const response = await fetch(
				`${this.deps.config.clickupApiBaseUrl}/space/${spaceId}/status`,
				{
					headers: {
						Authorization: `Bearer ${accessToken}`,
						"Content-Type": "application/json",
			...
			throw new Error(
				`ステータス一覧の取得に失敗しました: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	async getCustomFields(accessToken: string, listId: string) {
		try {
			const response = await fetch(
				`${this.deps.config.clickupApiBaseUrl}/list/${listId}/field`,
				{
					headers: {
						Authorization: `Bearer ${accessToken}`,
						"Content-Type": "application/json",
			...
			if (!response.ok) {
				throw new Error(
					`カスタムフィールド一覧の取得に失敗: ${response.status}`,
				);
			}
			...
			throw new Error(
				`カスタムフィールド一覧の取得に失敗しました: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
