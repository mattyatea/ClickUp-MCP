...
export class ClickUpTaskBasic {
   ...
   * @returns 更新されたタスク情報
   */
  async updateTask(accessToken: string, taskId: string, name?: string, description?: string, status?: string) {
   ...
   * @returns 更新されたタスク情報
   */
  async assignTask(accessToken: string, taskId: string, assigneeIds: string[], removeAssigneeIds?: string[]) {
    const updateData: any = {
      assignees: {
        add: assigneeIds,
      },
  ...
  async getTaskComments(accessToken: string, taskId: string) {
    const response = await fetch(`${this.deps.config.clickupApiBaseUrl}/task/${taskId}/comment`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      throw new Error(`ClickUpタスクのコメント取得に失敗しました: ${response.status}`);
    }
  ...
  async getSubtasks(accessToken: string, taskId: string, teamId: string) {
    const response = await fetch(`${this.deps.config.clickupApiBaseUrl}/team/${teamId}/task?parent=${taskId}&subtasks=true`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      throw new Error(`ClickUpサブタスクの取得に失敗しました: ${response.status}`);
    }
